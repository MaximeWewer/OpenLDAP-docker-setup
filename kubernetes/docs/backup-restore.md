...
BACKUP_POD=$(kubectl -n $NS run backup-shell \
  --image=alpine:3.19 --restart=Never --rm -it \
  --overrides='{"spec":{"containers":[{"name":"t","image":"alpine:3.19",
...
```bash
kubectl -n $NS debug --image=alpine:3.19 pod/ldap-openldap-0 \
       ...
       spec:
         ...
         - name: restore
           ...
           - |
             ...
             curl -sL https://github.com/maximewewer/openldap-cli/releases/download/v2026.6.3/openldap-cli_v2026.6.3_linux_amd64.tar.gz \
